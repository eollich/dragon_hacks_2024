...
Server::Server(int server_port)
    : port(server_port), conn(std::getenv("URI"), "foodr") {}
...
std::string Server::genToken() {
  ...
  if (users.find(str) != users.end()) {
    str = genToken(); // Update the token with the recursively generated unique
                      // token
  }
...
void Server::setupRoutes() {
  ...
  CROW_ROUTE(app, "/hello")([]() { return "Hello Hi Hi!"; });
  ...
  CROW_ROUTE(app, "/createUser")
      .methods(crow::HTTPMethod::Post)([this](const crow::request &req) {
        ...
        try {
          ...
          return crow::response(
              400, crow::json::wvalue({{"error", "invalid values"}}));
        }
        int added = conn.addUser(username.c_str(), password.c_str());
        if (added == 1) {
          return crow::response(
              400, crow::json::wvalue({{"error", "username already exists"},
                                       {"username", username}}));
        }

        return crow::response(200,
                              crow::json::wvalue({{"success", "user added"}}));
      });
  ...
  CROW_ROUTE(app, "/login")
      .methods(crow::HTTPMethod::Post)([this](const crow::request &req) {
        ...
        try {
          ...
          return crow::response(
              400, crow::json::wvalue({{"error", "invalid json key values"}}));
        }
        ...
        return crow::response(400,
                              crow::json::wvalue({{"error", "invalid login"}}));
      });
  ...
  CROW_ROUTE(app, "/deleteUser")
      .methods(crow::HTTPMethod::Post)([this](const crow::request &req) {
        ...
        if (removed == 0) {
          ...
          return crow::response(
              200, crow::json::wvalue({{"success", "account deleted"}}));
        }

        return crow::response(
            400, crow::json::wvalue({{"error", "could not delete account"}}));
      });

  CROW_ROUTE(app, "/getSwipeRestaurants")
      .methods(crow::HTTPMethod::Post)([this](const crow::request &req) {
        auto x = crow::json::load(req.body);
        if (!x) {
          return crow::response(
        ...
        try {
          ...
          visited_restaurants.insert(wanted_restaurants.begin(),
                                     wanted_restaurants.end());
          std::set_difference(all_restaurants.begin(), all_restaurants.end(),
                              visited_restaurants.begin(),
                              visited_restaurants.end(),
          ...
          return crow::response(
              400, crow::json::wvalue({{"error", "swipe curator error"}}));
        }
  ...
  CROW_ROUTE(app, "/getHaveVisit")
      .methods(crow::HTTPMethod::Post)([this](const crow::request &req) {
        ...
        try {
          ...
          std::cerr << e.what() << std::endl;
          return crow::response(
              400, crow::json::wvalue({{"error", "visited error"}}));
        }
        crow::json::wvalue jsonResult;
        size_t i = 0;
  ...
  ([&](const crow::request &req, std::string username) {
    std::vector<std::pair<std::string, double>> result;
    try {
      result = conn.getVisitedRestaurants(username);
    } catch (const std::exception e) {
      std::cerr << e.what() << std::endl;
      return crow::response(400,
                            crow::json::wvalue({{"error", "visited error"}}));
    }
    crow::json::wvalue jsonResult;
    size_t i = 0;
    for (const auto &pair : result) {
